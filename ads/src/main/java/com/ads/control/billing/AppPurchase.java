...
public class AppPurchase {
    ...
    private List<String> listSubscriptionId;
    ...
    BillingClientStateListener purchaseClientStateListener = new BillingClientStateListener() {
        ...
        public void onBillingSetupFinished(@NonNull BillingResult billingResult) {
            ...
            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                ...
                params.setSkusList(listSubscriptionId).setType(BillingClient.SkuType.SUBS);
                billingClient.querySkuDetailsAsync(params.build(), new SkuDetailsResponseListener() {
                    @Override
                    public void onSkuDetailsResponse(@NonNull BillingResult billingResult, @Nullable List<SkuDetails> list) {
                        if (list != null) {
    ...
    public void addSubscriptionId(String id) {
        if (listSubscriptionId == null)
            listSubscriptionId = new ArrayList<>();
        ...
        if (listINAPId != null) {
            billingClient.queryPurchasesAsync(BillingClient.SkuType.INAPP, new PurchasesResponseListener() {
                ...
                public void onQueryPurchasesResponse(@NonNull BillingResult billingResult, @NonNull List<Purchase> list) {
                    Log.d(TAG, "verifyPurchased IN-APP  code:" + billingResult.getResponseCode() + " ===   size:" + list.size());
                    if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && list != null) {
                        for (Purchase purchase : list) {
                            for (String id : listINAPId) {
                                if (purchase.getSkus().contains(id)) {
                                    Log.d(TAG, "verifyPurchased IN-APP: true");
                                    isPurchase = true;
                                    if (!verified) {
        ...
        if (listSubscriptionId != null) {
            billingClient.queryPurchasesAsync(BillingClient.SkuType.SUBS, new PurchasesResponseListener() {
                ...
                public void onQueryPurchasesResponse(@NonNull BillingResult billingResult, @NonNull List<Purchase> list) {
                    ...
                    if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && list != null) {
                        for (Purchase purchase : list) {
                            for (String id : listSubscriptionId) {
                                if (purchase.getSkus().contains(id)) {
...
//        AdjustTL.onTrackRevenuePurchase((float) price, currency);
